...
const UserRequireController = {
  async createUserRequire(req, res) {
    try {
      const { userId, category, budget, totalDay, address } = req.body;

      // Validate required fields
      if (!userId || !category || !budget || !totalDay || !address) {
        return res.status(400).json({
          type: "error",
          message: "All fields are required (userId, category, budget, totalDay, address)",
        });
      }
